...
function translateGasEstimates(gasEstimates: number | any[]) {
      ...
      evm: {
        ...
        deployedBytecode: {
          object:
            ...
            linker.linkBytecode(contractInput.runtimeBytecode, libraries || {}),
...
function formatAssemblyText(asm: string, prefix: string, source: string) {
  if (typeof asm === 'string' || asm === null || asm === undefined) {
    return prefix + (asm || '') + '\n'
  }
  let text = prefix + '.code\n'
  // @ts-ignore
  asm['.code'].forEach((item, i) => {

 ...
 */
function libraryHashPlaceholder(fullyQualifiedLibraryName: string) {
  return `$${keccak_256(fullyQualifiedLibraryName).slice(0, 34)}$`
}
 ...
 */
function replacePlaceholder(bytecode: string, label: string, address: string) {
...
function linkBytecode(bytecode: string, libraries: LibraryAddresses): string {
  ...
  for (const [
    fullyQualifiedLibraryName,
    libraryObjectOrAddress
  ] of Object.entries(libraries)) {
    if (isNil(libraryObjectOrAddress)) {
      throw new Error(
        `No address provided for library ${fullyQualifiedLibraryName}`
      )
    }
    ...
    if (isObject(libraryObjectOrAddress)) {
      for (const [unqualifiedLibraryName, address] of Object.entries(
        ...
        librariesComplete[
          `${fullyQualifiedLibraryName}:${unqualifiedLibraryName}`
        ] = address
      }
    ...
    const parsed = fullyQualifiedLibraryName.match(
      /^(?<sourceUnitName>[^:]+):(?<unqualifiedLibraryName>.+)$/
    )
...
export default { linkBytecode, findLinkReferences }
